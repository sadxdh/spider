...
def producer():
    df = pd.read_excel('./@yunhzz_user_tweets.xlsx')
    Media_URLs_df = df[(df['Media Type'] == 'video')].loc[:, 'Media URLs':]
    # Media_URLs_df
    output_lst = list(Media_URLs_df['Media URLs'])

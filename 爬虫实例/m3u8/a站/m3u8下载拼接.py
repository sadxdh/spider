...
def join_ts1(video_name):
    ...
    title_list = [f"{t}.ts" for t in title_list]
    # title_list = [f"{i}.ts" for i in range(1, 15 + 1)]
    # # print(title_list)
    s = "+".join(title_list)
...
async def download_ts(url, title, session):
    async with session.get(url) as resp3:
        async with aiofiles.open(f"{title}.ts", 'wb') as f:
            print(f)
...
if __name__ == '__main__':
    video_name = '里世界郊游 ：第1话 扭来扭去的狩猎'  # .m3u8         # 影片名
    ...
    up_url = "https://tx-safety-video.acfun.cn/mediacloud/acfun/acfun_video/hls/"  # 需要绝对路径
